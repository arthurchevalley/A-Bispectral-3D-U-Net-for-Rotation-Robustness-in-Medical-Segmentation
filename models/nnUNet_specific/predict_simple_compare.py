        ...
        try:
            ...
            if segs_from_prev_stage[i] is not None:
                assert isfile(segs_from_prev_stage[i]) and segs_from_prev_stage[i].endswith(
                    ".npz"), "segs_from_prev_stage" \
                                " must point to a " \
                                "segmentation file"
                seg_prev = np.load(segs_from_prev_stage[i])#sitk.GetArrayFromImage(sitk.ReadImage(segs_from_prev_stage[i]))
                # check to see if shapes match
                img = np.load(l[0]) #sitk.GetArrayFromImage(sitk.ReadImage(l[0]))
...
def main():
    print("!!Using a modified non-official predicting script!!")
    parser = argparse.ArgumentParser()
    parser.add_argument("-i", '--input_folder', help="Must contain all modalities for each patient in the correct"
                                                     " order (same as training). Files must be named "
                                                     "CASENAME_XXXX.npz where XXXX is the modality "
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    all_files = subfiles(input_folder, suffix=".npz", join=False, sort=True)
    list_of_lists = [join(input_folder, i) for i in all_files]
    list_of_lists = [[join(input_folder, i) for i in all_files] for j in case_ids]


    if lowres_segmentations is not None:
        assert isdir(lowres_segmentations), "if lowres_segmentations is not None then it must point to a directory"
        lowres_segmentations = [join(lowres_segmentations, i + ".npz") for i in case_ids]
        assert all([isfile(i) for i in lowres_segmentations]), "not all lowres_segmentations files are present. " \
                                                               "(I was searching for case_id.npz in that folder)"
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    :param list_of_lists: [[case0_0000.npz, case0_0001.npz], [case1_0000.npz, case1_0001.npz], ...]
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    ...
    files = subfiles(input_folder, suffix=".npz", join=False, sort=True)

    maybe_case_ids = np.unique([i[:-4] for i in files])


    assert len(files) > 0, "input folder did not contain any images (expected to find .npz file endings)"

    ''' # now check if all required files are present and that no unexpected files are remaining
    for c in maybe_case_ids:
        for n in range(expected_num_modalities):
            expected_output_file = c + "_%04.0d.npz" % n
    ...
    print("If they don't look right, make sure to double check your filenames. They must end with _0000.npz etc")
